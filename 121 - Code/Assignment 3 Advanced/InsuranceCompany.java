...
public class InsuranceCompany implements Cloneable, Serializable{
  ...
  void setFlatRate(int flatRate){
      this.flatRate = flatRate;
  }

  void setAdminUsername(String adminUsername){
      this.adminUsername = adminUsername;
      System.out.println("Passed: " + adminUsername + ", Saved: " + this.adminUsername);
  }
 ...
 boolean validateAdmin(String username, String password){
  ...
  Boolean addUser(User user){
    if (users == null){
      ...
      users.put(user.getUserID(), user);
      System.out.println("User added to users hash");
    } else {
          System.out.println("userID collision, userID: " + user.getUserID());
      }
    } 
...
//  }
    ...
    double calcTotalPayments(String userName, String userPassword){
        ...
        getUserIDs().forEach(key -> {
            runningTotal[0] += users.get(key).calcTotalPremiums(flatRate, userName, userPassword);
        });
...
//  }
    ...
    void carPriceRise(double risePercent, String userName, String userPassword){
        users.forEach((userID, user) -> user.carPriceRiseAll(risePercent, userName, userPassword));
    }
...
//  }
    ...
    HashMap<Integer, InsurancePolicy> filterByCarModel(String carModel){
        ...
        users.forEach((userID, user) -> {
            ...
            modelPolicies.putAll(user.filterByCarModel(carModel, userName, userPassword));
        });
...
//  }
    ...
    ArrayList<Double> getTotalPaymentsPerCity(ArrayList<String> cities){
        ...
        cities.forEach(city -> {
            double oneCity = getTotalPaymentsForCity(city);
            totalPerCity.add(oneCity);
        });
        return totalPerCity;
    }
  ...
  void reportPaymentPerCity(ArrayList<String> cities, ArrayList<Double> payments){
    ...
    String reportPaymentPerCityToString(){

        ArrayList<String> cities = populateDistinctCityNames();
        ArrayList<Double> payments = getTotalPaymentsPerCity(cities);
      String output = "";
        output += ("City Name, " + "Total Monthly Payment" + "\n");


        //System.out.println("CIty Name           Total Premium Payment");
        for (int i=0; i < cities.size(); i++){
            //System.out.print(cities.get(i) + payments.get(i));
            output += (cities.get(i) + ", "+ "$" +payments.get(i) + "\n");
        }
        return output;
    }
  ...
  */
    ...
    void reportPaymentsPerCarModel(ArrayList<String> carModels, ArrayList<Integer> counts, ArrayList<Double> premiumPayments){
        ...
        carModels.forEach(model -> {
            int position = carModels.indexOf(model);
            double calcAvg = (premiumPayments.get(position) / counts.get(position));
            System.out.format(format, carModels.get(position), premiumPayments.get(position), calcAvg);
        });
    ...
    String reportPaymentsPerCarModelToString(){
        String output = "";
        ...
        output += ("Car Model, " + "Total Premium Payment, " + "Average Premium Payment" + "\n");

        for (int i = 0; i < carModels.size(); i++){
            double calcAvg = (premiumPayments.get(i) / counts.get(i));

            output += (carModels.get(i) + ", $" + premiumPayments.get(i) + ", $" + calcAvg + "\n");
        }
    return output;
    }
   ...
   */
    ...
    public void reportPaymentPerCity(){
        ...
        cityPayments.forEach((city, payment) -> {
            System.out.format(format, city, payment);
        });
    }
  ...
  public Boolean load (String fileName, String adminUsername, String adminPassword) throws IOException, CloneNotSupportedException{
    ...
    try {
      if (input != null){
        ...
        this.users = deepCopyUsersHashMap(loadedCompany.getUsersHash());
...
//Working
  ...
  HashMap<Integer, InsurancePolicy> getPoliciesHash(){
    ...
    ArrayList<InsurancePolicy> allPoliciesArray(){
        if (adminUsername.equals(getAdminUserName()) && adminPassword.equals(getAdminPassword())){
            Set<Integer> userKeys = users.keySet();
            ...
            users.forEach((userID, user) -> {
                ...
                userPolicies.forEach(policy -> allPolicies.add(policy));
            });
...
//Password
  ...
  int userIDGen(){
    ...
    int lowerBound = 40000;
     ...
     userIDGen = random.nextInt(upperBound - lowerBound + 1) + lowerBound;
  ...
  */
    ...
    public int[] userCount(String adminUsername, String adminPassword, int[] ranges) {
        if (adminUsername.equals(getAdminUserName()) && adminPassword.equals(getAdminPassword())) {
            ...
            for (int i = 0; i < ranges.length - 1; i++) {
                final int finali = i;
                int count = userKeys.stream()
                        .flatMap(user -> users.get(user).getHashPolices(users.get(user).getUserName(), users.get(user).getUserPassword()).values().stream())
                        .mapToInt(policy -> {
                            double calcPayment = policy.calcPayment(InsuranceCompany.flatRateStatic);
                            return (calcPayment >= ranges[finali] && calcPayment < ((finali == ranges.length - 1) ? ranges[finali - 1] : ranges[finali + 1])) ? 1 : 0;
                        })
                        .sum();
...
public int findUserID(int policyID){
      Set<Integer> userKeys = users.keySet();
      int foundUserID = 0;
      for (Integer user: userKeys){
          String userName = users.get(user).userName;
          String userPassword = users.get(user).userPassword;
          HashMap<Integer, InsurancePolicy> userPolicies = users.get(user).getHashPolices(userName, userPassword);
          Set<Integer> policyKeys = userPolicies.keySet();
          for (Integer policy: policyKeys){
              if (policy == policyID){
                  foundUserID = user;

...
public static void AdminMenu(InsuranceCompany insuranceCompany, ArrayList<InsurancePolicy> policies) throws IOException, CloneNotSupportedException, PolicyException, NameException{
  ...
  String uPassword = "Admin";
    ...
    while (logOut == 0){
      ...
      System.out.println("18) SQL Database save");
      System.out.println("19) SQL Database load");
      ...
      //inputMis
        ...
        if (insuranceCompany.removeUser(1) == true){
        ...
        firstTime = false;
        SQLMain.saveToDB(insuranceCompany);
      } else if (option == 19){
        firstTime = false;
        SQLMain.loadFromDB(insuranceCompany);
        }
       else if (option == 20){
...
//inputMis
  ...
  public static void testCase(InsuranceCompany insuranceCompany) throws IOException, CloneNotSupportedException, PolicyException, NameException{
    ...
    //InsuranceCompany insuranceCompany = new InsuranceCompany("Matthew", user1, "Admin", "Admin", 600);
      System.out.println("Loaded insuranceCompany is: " + insuranceCompany.name);
      insuranceCompany.setName("Admin");
      insuranceCompany.setAdminPassword("Admin");
      insuranceCompany = new InsuranceCompany("Matthew", new User("Admin", 600, new Address(6, "CompanyStreet", "CompanySuburb", "CompanyCity"), "Admin", "Admin"), "Admin", "Admin", 100);
    if (!insuranceCompany.validateAdmin(insuranceCompany.getAdminUserName(), insuranceCompany.getAdminPassword())){
    ...
    try 
    {
      User.save(insuranceCompany.getUsersHash(),"users.ser");
    ...
    try 
    {
      User.saveTextFile(insuranceCompany.getUsersHash(),"users.txt");
  ...
  public static void LabEight(InsuranceCompany insuranceCompany) throws IOException, CloneNotSupportedException, PolicyException, NameException{
      ...
      if (!insuranceCompany.validateAdmin("Admin", "Admin")){
        ...
        ArrayList<InsurancePolicy> policies = insuranceCompany.allPoliciesArray();
        System.out.println("All policies size: " + policies.size());

        System.out.println("Part A/B");
        policies.forEach(policy -> {
          if(policy.policyHolderName.equals("Santa Month")){
            InsurancePolicy.printPolicies(policy, insuranceCompany.flatRate);
          }
        });
        System.out.println();
        ...
        Optional<InsurancePolicy> firstPolicyInRange = policies.stream()
                .filter(x -> x.getPolicyPremium(insuranceCompany.flatRate) >= 200 && x.getPolicyPremium(insuranceCompany.flatRate) <= 500)
                .findFirst();

        firstPolicyInRange.ifPresent(System.out::println);
        System.out.println();

        System.out.println("Part D");
        ArrayList<InsurancePolicy> allPoliciesD = new ArrayList<>();
        policies.stream()
                .filter(x -> x.getPolicyPremium(insuranceCompany.flatRate) >= 200 && x.getPolicyPremium(insuranceCompany.flatRate) <= 500)
                .forEach(allPoliciesD::add);
        InsurancePolicy.printPolicies(allPoliciesD, insuranceCompany.flatRate);
        System.out.println();

        System.out.println("Part E");
        ArrayList<InsurancePolicy> allPoliciesE = new ArrayList<>();
        policies.stream()
                .filter(x -> x.getPolicyPremium(insuranceCompany.flatRate) >= 200 && x.getPolicyPremium(insuranceCompany.flatRate) <= 500)
                .forEach(allPoliciesE::add);
        ArrayList<Double> resultE = new ArrayList<>();
        allPoliciesE.forEach(policy -> resultE.add(policy.calcPayment(insuranceCompany.flatRate)));
        double sum = resultE.stream().mapToDouble(Double::doubleValue).sum();
        System.out.println("Sum of policies between 200 and 500: $" + sum);
        System.out.println();
      ...
      //filterPolicies(policies, Car.CarType.LUX);
        System.out.println();

        System.out.println("Part H");
        //Collectors.groupingBy(policies)
  ...
  public static ArrayList<InsurancePolicy> filterPolicies (ArrayList<InsurancePolicy> policies, Predicate<InsurancePolicy> criteria){
    Predicate<InsurancePolicy> c1= x -> x.getPolicyHolderName().equals("Santa Month");
    ...
    InsurancePolicy.printPolicies (filterPolicies(policies, x->x.getCarModel(policies).contains("Toyota")));
    return policies1;
  }

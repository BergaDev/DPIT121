  ...
  */
  ...
  int [] policyCount (String uName, String uPassword, int [] ranges){
    if (uName.equals(getUserName()) && uPassword.equals(getUserPassword())) {
      ...
      for (int i = 0; i < ranges.length; i++) {
        int lower = ranges[i];
        int higher = (i == ranges.length - 1) ? ranges[i - 1] : ranges[i + 1];

        int count = policyKeys.stream()
                .filter(policy -> {
                  double calcPayment = policies.get(policy).calcPayment(InsuranceCompany.flatRateStatic);
                  return calcPayment >= lower && calcPayment < higher;
                })
                .mapToInt(policy -> 1)
                .sum();
  ...
  public HashMap<String, Integer[]> policyCarModelCount(String username, String password, Integer[] ranges) {
    if (username.equals(getUserName()) && password.equals(getUserPassword())) {
      HashMap<String, Integer[]> outputHash = new HashMap<>();
      Set<Integer> policyKeys = policies.keySet();
      ArrayList<String> carModels = distinctCarModels();
      ...
      carModels.forEach(model -> {
        ...
        for (int i = 0; i < ranges.length; i++) {
          ...
          int count = policyKeys.stream()
                  .filter(policy -> {
                    InsurancePolicy policyObject = policies.get(policy);
                    return model.equals(policyObject.getCar().getModel()) &&
                            policyObject.calcPayment(InsuranceCompany.flatRateStatic) >= ranges[finalInt] &&
                            policyObject.calcPayment(InsuranceCompany.flatRateStatic) < ((finalInt == ranges.length - 1) ? ranges[finalInt - 1] : ranges[finalInt + 1]);
                  })
                  .mapToInt(policy -> 1)
                  .sum();
